import bisect
import heapq
from datetime import datetime, timezone
from typing import Any

from fastapi import APIRouter, Body

router = APIRouter()

NEG_INF = float("-inf")
POS_INF = float("inf")
EPS = 1e-6


def parse_iso(s: str) -> float:
  if s.endswith("Z"):
    s = s[:-1] + "+00:00"
  dt = datetime.fromisoformat(s)
  if dt.tzinfo is None:
    dt = dt.replace(tzinfo=timezone.utc)
  return dt.timestamp()


def format_iso(t: float) -> str:
  t_rounded = round(t, 3)
  dt = datetime.fromtimestamp(t_rounded, tz=timezone.utc)
  if dt.microsecond == 0:
    return dt.strftime("%Y-%m-%dT%H:%M:%SZ")
  ms = dt.microsecond // 1000
  return dt.strftime("%Y-%m-%dT%H:%M:%S") + f".{ms:03d}Z"


def build_timeline(obs_list):
  """obs_list: list of (start_ts, end_ts, speed_factor). Returns (bounds, rates)
  where rates[i] is the effective speed factor active during [bounds[i], bounds[i+1]).
  Overlapping obstructions combine via min() (most restrictive wins)."""
  if not obs_list:
    return None
  pts = sorted(set([s for s, e, f in obs_list] + [e for s, e, f in obs_list]))
  bounds = [NEG_INF] + pts + [POS_INF]
  rates = [1.0]
  for i in range(len(pts) - 1):
    lo = pts[i]
    active = [f for (s, e, f) in obs_list if s <= lo < e]
    rates.append(min(active) if active else 1.0)
  rates.append(1.0)
  return bounds, rates


def traverse(timeline, duration, entry_time):
  """Simulate traversing an edge of length `duration` (in normal-speed seconds)
  starting at entry_time, given a piecewise speed-factor timeline. Returns the
  exit time, or None if the traversal would require waiting (speed factor hits
  0 before the edge is completed) - waiting is never allowed, mid-edge or not."""
  if duration <= 0:
    return entry_time
  if timeline is None:
    return entry_time + duration

  bounds, rates = timeline
  i = bisect.bisect_right(bounds, entry_time) - 1
  if i < 0:
    i = 0
  n = len(rates)
  if i >= n:
    i = n - 1

  remaining = duration
  cur = entry_time
  while True:
    if remaining <= EPS:
      return cur
    rate = rates[i]
    if rate <= 0:
      return None
    seg_end = bounds[i + 1]
    if seg_end == POS_INF:
      return cur + remaining / rate
    avail = seg_end - cur
    progress = rate * avail
    if progress >= remaining - EPS:
      return cur + remaining / rate
    remaining -= progress
    cur = seg_end
    i += 1
    if i >= n:
      return None


def static_dijkstra(adj, source):
  """Shortest (static, obstruction-free) distance from every node to `source`,
  computed via one Dijkstra run rooted at `source` over the symmetric graph."""
  dist = {source: 0.0}
  prev = {}
  pq = [(0.0, source)]
  visited = set()
  while pq:
    d, u = heapq.heappop(pq)
    if u in visited:
      continue
    visited.add(u)
    for (v, eid, w) in adj.get(u, []):
      nd = d + w
      if v not in dist or nd < dist[v] - EPS:
        dist[v] = nd
        prev[v] = (u, eid)
        heapq.heappush(pq, (nd, v))

  def get_path(node):
    if node not in dist:
      return None
    path = []
    cur = node
    while cur != source:
      if cur not in prev:
        return None
      pu, eid = prev[cur]
      path.append(eid)
      cur = pu
    return path

  return dist, get_path


UNREACHABLE = {"total_duration_sec": None, "arrival_time": None, "path": []}
MAX_STATES = 200_000


def solve_case(case: dict) -> dict:
  try:
    start = tuple(case["start_coordinate"])
    end = tuple(case["end_coordinate"])
    t0 = parse_iso(case["start_time"])
    edges = case.get("edges", [])
    obstructions = case.get("obstructions", [])

    directed_from: dict = {}
    edge_lookup: dict = {}
    static_adj: dict = {}

    for e in edges:
      n1 = tuple(e["node1"])
      n2 = tuple(e["node2"])
      eid = e["edge_id"]
      dur = e["base_duration_sec"]
      for (a, b) in ((n1, n2), (n2, n1)):
        rec = {"from": a, "to": b, "edge_id": eid, "dur": dur, "obs": [], "timeline": None}
        directed_from.setdefault(a, []).append(rec)
        edge_lookup[(eid, a, b)] = rec
      static_adj.setdefault(n1, []).append((n2, eid, dur))
      static_adj.setdefault(n2, []).append((n1, eid, dur))

    T_max = t0
    for obs in obstructions:
      eid = obs["edge_id"]
      f = tuple(obs["edge"]["from"])
      tt = tuple(obs["edge"]["to"])
      rec = edge_lookup.get((eid, f, tt))
      if rec is None:
        continue
      s = parse_iso(obs["start_time"])
      e_ = parse_iso(obs["end_time"])
      sf = float(obs["speed_factor"])
      rec["obs"].append((s, e_, sf))
      if e_ > T_max:
        T_max = e_

    for lst in directed_from.values():
      for rec in lst:
        if rec["obs"]:
          rec["timeline"] = build_timeline(rec["obs"])

    static_dist, get_path = static_dijkstra(static_adj, end)

    if start == end:
      return {"total_duration_sec": 0, "arrival_time": format_iso(t0), "path": []}

    def tkey(t):
      return round(t, 6)

    counter = 0
    start_key = (start, tkey(t0))
    came_from = {start_key: None}
    visited = set()
    heap = [(t0, counter, start)]
    states_processed = 0

    while heap:
      t, _, node = heapq.heappop(heap)
      key = (node, tkey(t))
      if key in visited:
        continue
      visited.add(key)
      states_processed += 1
      if states_processed > MAX_STATES:
        break

      if node == end:
        edges_rev = []
        cur = key
        while came_from[cur] is not None:
          pk, info = came_from[cur]
          if isinstance(info, tuple):
            sp = get_path(info[1]) or []
            edges_rev.extend(reversed(sp))
          else:
            edges_rev.append(info)
          cur = pk
        path = list(reversed(edges_rev))
        total = t - t0
        total_out = int(round(total)) if abs(total - round(total)) < 1e-6 else round(total, 6)
        return {"total_duration_sec": total_out, "arrival_time": format_iso(t), "path": path}

      if t >= T_max - EPS:
        sd = static_dist.get(node)
        if sd is not None and sd < POS_INF:
          arrival = t + sd
          nk = (end, tkey(arrival))
          if nk not in visited:
            if nk not in came_from:
              came_from[nk] = (key, ("STATIC", node))
            counter += 1
            heapq.heappush(heap, (arrival, counter, end))
        continue

      for rec in directed_from.get(node, []):
        exit_t = traverse(rec["timeline"], rec["dur"], t)
        if exit_t is None:
          continue
        nk = (rec["to"], tkey(exit_t))
        if nk in visited:
          continue
        if nk not in came_from:
          came_from[nk] = (key, rec["edge_id"])
        counter += 1
        heapq.heappush(heap, (exit_t, counter, rec["to"]))

    return dict(UNREACHABLE)
  except Exception:
    return dict(UNREACHABLE)


@router.post("/kan-cheong-delivery-driver")
def kan_cheong_delivery_driver(payload: dict = Body(...)) -> Any:
  return {case_id: solve_case(case) for case_id, case in payload.items()}
